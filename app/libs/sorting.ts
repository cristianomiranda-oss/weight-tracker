...
// Based merge sort functions are based on their initial code but have been adapted to work with different object types
 ...
 /**
       ...
       */
export function sortEntriesArray(entriesArray: WeightEntryType[], comparisonKey: keyof WeightEntryType, sortOrder: sortOrder) {
    /**
       ...
       */
    function mergeEntries(entriesArray: WeightEntryType[], left: number, mid: number, right: number) {
    ...
    /**
       ...
       * @param right End position of the elements to be sorted
       */
    function mergeSortEntries(entriesArray: WeightEntryType[], left: number, right: number) {
        if (left >= right) {
